...
Exposes the synchronous logic in ``services.tasmota_runtime`` over REST so the
...
from ..services import tasmota_runtime
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.restart_device, get_db_path(request), mac, settings, use_mqtt
            )
        except tasmota_runtime.TasmotaRuntimeError as exc:
...
class TasmotaUpgradeAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.upgrade_device, get_db_path(request), mac, settings, use_mqtt
            )
        except tasmota_runtime.TasmotaRuntimeError as exc:
            return self.json({"error": str(exc)}, status_code=502)
        await emit_activity_log(
...
class TasmotaStatusAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.get_status, get_db_path(request), mac, settings
            )
        except tasmota_runtime.TasmotaRuntimeError as exc:
            return self.json({"error": str(exc)}, status_code=502)
        return self.json(result, status_code=200)
...
class TasmotaSwitchApAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.switch_ap, get_db_path(request), mac, settings, ap_id
            )
        except tasmota_runtime.TasmotaRuntimeError as exc:
            return self.json({"error": str(exc)}, status_code=502)
        await emit_activity_log(
...
class TasmotaCheckUnavailableAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.check_unavailable, get_db_path(request), settings, mac_filter
            )
        except tasmota_runtime.BatchInProgressError as exc:
...
class TasmotaForceApAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.force_ap_batch, get_db_path(request), settings, ap_id, mac_filter
            )
        except tasmota_runtime.BatchInProgressError as exc:
            return self.json({"error": str(exc)}, status_code=409)
        await emit_activity_log(
...
class TasmotaUpdateFirmwareAPIView(BaseView):
    ...
    async def post(self, request):
        ...
        try:
            result = await hass.async_add_executor_job(
                tasmota_runtime.update_firmware_batch,
                get_db_path(request),
                settings,
                str(target_version),
                mac_filter,
