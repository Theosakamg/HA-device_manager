...
Bridges HA service calls to the synchronous logic in ``tasmota_runtime`` by:
...
from ..utils.tasmota_target import TasmotaTargetError, resolve_device_id_to_mac
from . import tasmota_runtime
...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    async def _restart(call: ServiceCall) -> ServiceResponse:
        ...
        result = await hass.async_add_executor_job(
            tasmota_runtime.restart_device, _db_path(hass), mac, settings, use_mqtt
        )
    ...
    async def _upgrade(call: ServiceCall) -> ServiceResponse:
        ...
        use_mqtt = bool(call.data.get("use_mqtt", False))
        result = await hass.async_add_executor_job(
            tasmota_runtime.upgrade_device, _db_path(hass), mac, settings, use_mqtt
        )
    ...
    async def _status(call: ServiceCall) -> ServiceResponse:
        ...
        settings = await _load_settings(hass)
        result = await hass.async_add_executor_job(
            tasmota_runtime.get_status, _db_path(hass), mac, settings
        )
        return dict(result)
    ...
    async def _switch_ap(call: ServiceCall) -> ServiceResponse:
        ...
        result = await hass.async_add_executor_job(
            tasmota_runtime.switch_ap, _db_path(hass), mac, settings, ap_id
        )
    ...
    async def _check_unavailable(call: ServiceCall) -> ServiceResponse:
        ...
        result = await hass.async_add_executor_job(
            tasmota_runtime.check_unavailable, _db_path(hass), settings, mac_filter
        )
        return dict(result)
    ...
    async def _force_ap(call: ServiceCall) -> ServiceResponse:
        ...
        mac_filter = call.data.get("mac_filter") or None
        result = await hass.async_add_executor_job(
            tasmota_runtime.force_ap_batch, _db_path(hass), settings, ap_id, mac_filter
        )
