...
import { type GraphNode, type GraphEdge, type MapStats, COLORS, REF_EDGE_COLORS } from "./map-constants";
...
export class DmMapView extends LitElement {
  ...
  disconnectedCallback() {
    ...
    this._vcRenderer?.dispose(); this._vcRenderer?.forceContextLoss();
    this._nodes = []; this._edges = [];
    this._hoveredNode = null; this._focusedNode = null;
    this._focusRelatedCache = null; this._flyAnimating = false;
  ...
  private async _init() {
    ...
    try {
      ...
      if (dbg) {
        ...
        this._gpuRenderer = this._renderer.capabilities.isWebGL2 ? "WebGL2" : "WebGL1";
    ...
    this._camera = new THREE.PerspectiveCamera(55, wrap.clientWidth / wrap.clientHeight, 0.1, 500);
    this._camera.position.set(0, 24, 55);

    this._controls = new OrbitControls(this._camera, canvas);
    Object.assign(this._controls, { enableDamping: true, dampingFactor: 0.08, minDistance: 8, maxDistance: 180, autoRotate: true, autoRotateSpeed: 0.4 });

    this._scene.add(new THREE.AmbientLight(0xffffff, 0.75));
    const p1 = new THREE.PointLight(0xfbbf24, 1.2, 120); p1.position.set(20,  30,  20);  this._scene.add(p1);
    const p2 = new THREE.PointLight(0x6366f1, 0.8, 120); p2.position.set(-20, -10, -20); this._scene.add(p2);
  ...
  private _buildGraph(
    tree: HierarchyTree, devices: DmDevice[],
    firmwares: DmDeviceFirmware[], models: DmDeviceModel[], functions: DmDeviceFunction[],
  ) {
    const { nodes, edges, stats } = buildGraphData(tree, devices, firmwares, models, functions, this._layoutMode);
    this._nodes = nodes; this._edges = edges; this._stats = stats;
    renderGraphToScene(this._scene, this._nodes, this._edges, this._renderer.capabilities.getMaxAnisotropy());
  }
  ...
  private _focusOn(node: GraphNode) {
    ...
    const relevant = this._nodes.filter((n) => n.id === node.id || related.has(n.id));

    const center = relevant.reduce((c, n) => c.add(n.position), new THREE.Vector3()).divideScalar(relevant.length || 1);
    let maxDist = 0;
    for (const n of relevant) maxDist = Math.max(maxDist, n.position.distanceTo(center));
    ...
    const targetPos = center.clone().add(dir.multiplyScalar(Math.min(Math.max(fitDist, 8), 120)));
  ...
  private _releaseFocus() {
    if (this._focusedNode?.mesh)
      (this._focusedNode.mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.25;
    this._clearFocusDimming();
    this._removeFocusRing();
    this._focusedNode = null; this._focusRelatedCache = null;
  ...
  private _getRelatedNodeIds(nodeId: string): Set<string> {
    ...
    for (const e of this._edges) {
      (childrenOf.get(e.source) ?? (childrenOf.set(e.source, []), childrenOf.get(e.source)!)).push(e.target);
      (parentsOf.get(e.target)  ?? (parentsOf.set(e.target,  []), parentsOf.get(e.target)!)).push(e.source);
    }
    const bfs = (start: string, map: Map<string, string[]>) => {
      const q = [start];
      while (q.length)
        for (const n of map.get(q.shift()!) ?? [])
          if (!related.has(n)) { related.add(n); q.push(n); }
    };
  ...
  private _applyFocusDimming(focusedId: string) {
    ...
    for (const node of this._nodes) {
      ...
      if (node.id === focusedId) {
        mat.emissiveIntensity = 1; mat.opacity = 1; mat.depthWrite = true;
  ...
  private _clearFocusDimming() {
    for (const node of this._nodes) {
      if (!node.mesh) continue;
      const mat = node.mesh.material as THREE.MeshStandardMaterial;
      mat.emissiveIntensity = 0.25; mat.opacity = 1; mat.depthWrite = true;
      if (node.labelSprite) node.labelSprite.material.opacity = 1;
    }
    for (const edge of this._edges) {
      if (!edge.line) continue;
      ...
      mat.color.set(isRef ? (REF_EDGE_COLORS[edge.edgeType!] ?? 0x64748b) : 0x64748b);
    }
  }
  ...
  private _initViewCube(wrap: HTMLDivElement) {
    ...
    vcDir.position.set(2, 3, 4); this._vcScene.add(vcDir);

    const faceLabels = ["Right", "Left", "Top", "Bottom", "Front", "Back"];
    this._vcCube = new THREE.Mesh(
      new THREE.BoxGeometry(1.6, 1.6, 1.6),
      faceLabels.map((l) => new THREE.MeshStandardMaterial({ map: this._makeVCFaceTexture(l), roughness: 0.6, metalness: 0.1 })),
    );
    ...
    vcWrap.addEventListener("click", (e: MouseEvent) => {
      ...
      vcMouse.set(((e.clientX - rect.left) / rect.width) * 2 - 1, -((e.clientY - rect.top) / rect.height) * 2 + 1);
  ...
  private _makeVCFaceTexture(label: string): THREE.CanvasTexture {
    ...
    ctx.fillStyle = "rgba(255,255,255,0.88)"; ctx.fillRect(0, 0, size, size);
    ctx.strokeStyle = "#94a3b8"; ctx.lineWidth = 4; ctx.strokeRect(2, 2, size - 4, size - 4);
    ctx.fillStyle = "#1e293b"; ctx.font = "bold 72px Arial, sans-serif";
  ...
  private _flyToViewCubeFace(faceIdx: number) {
    ...
    this._controls.autoRotate = false;
    this._startFly(target.clone().add(dir.multiplyScalar(this._camera.position.distanceTo(target))), target);
  }
  ...
  private _animate = () => {
    ...
    const t = this._clock.getElapsedTime(); this._clock.getDelta();

    // Fly-to
    if (this._flyAnimating) {
      const ease = easeInOut(Math.min((t - this._flyStartTime) / this._flyDuration, 1));
      this._camera.position.lerpVectors(this._flyFrom, this._flyTo, ease);
      this._controls.target.lerpVectors(this._flyFromLookAt, this._flyTargetLookAt, ease);
    ...
    for (const node of this._nodes) {
      ...
      if (node.labelSprite) node.labelSprite.position.y = node.mesh.position.y + node.radius + 1.2;
    }

    // Focus ring pulse
    if (this._focusRing && this._focusedNode?.mesh) {
      this._focusRing.position.copy(this._focusedNode.mesh.position);
      (this._focusRing.material as THREE.MeshBasicMaterial).opacity = 0.25 + Math.sin(t * 3) * 0.1;
    ...
    const meshes  = this._nodes.map((n) => n.mesh).filter(Boolean) as THREE.Mesh[];
    ...
    if (hits.length > 0) {
      ...
      if (node && node !== this._hoveredNode) {
        if (this._hoveredNode?.mesh && this._hoveredNode !== this._focusedNode)
          (this._hoveredNode.mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = this._getBaseEmissive(this._hoveredNode);
        this._hoveredNode = node;
        if (!this._focusedNode || node === this._focusedNode || this._getBaseEmissive(node) > 0)
          ((hits[0].object as THREE.Mesh).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.65;
      }
      if (tooltip && node) {
    ...
    if (now - this._dbgLastTime >= 1000) {
      ...
      this._dbgFrameCount = 0; this._dbgLastTime = now;
  ...
  private _onMouseDown = (e: MouseEvent) => { this._mouseDownPos = { x: e.clientX, y: e.clientY }; };
  ...
  private _handleNodeClick() {
    ...
    if (!hits.length) { if (this._focusedNode) this._releaseFocus(); return; }

    const nodeId = (hits[0].object as THREE.Mesh).userData.nodeId as string;
    const node   = this._nodes.find((n) => n.id === nodeId);
    if (!node) return;
    if (this._focusedNode === node) { this._releaseFocus(); return; }

    if (this._focusedNode?.mesh)
      (this._focusedNode.mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.25;
    this._clearFocusDimming();
    this._focusedNode = node; this._focusRelatedCache = null;
  ...
  private _toggleDebug = () => {
    ...
    if (this._debugVisible) requestAnimationFrame(() => this._updateDebugOverlay());
  };

  private _updateDebugOverlay() {
    const overlay = this.shadowRoot?.querySelector<HTMLDivElement>(".debug-overlay");
    if (!overlay) return;
    ...
    const swKeys  = ["llvmpipe", "softpipe", "swrast", "software", "microsoft basic"];
    const isSW    = swKeys.some((k) => this._gpuRenderer.toLowerCase().includes(k));
    ...
    overlay.innerHTML = `
      ...
      <hr class="debug-sep"/>
      ${mem ? `<div class="debug-row"><span class="debug-key">Mémoire JS</span><span class="debug-val">${(mem.usedJSHeapSize / 1024 / 1024).toFixed(1)} MB</span></div>
  ...
  render() {
    const legendItems: [string, string][] = [
      ["#fbbf24", i18n.t("home")],  ["#14b8a6", i18n.t("level")],
      ["#6366f1", i18n.t("room")],  ["#64748b", i18n.t("device")],
    ...
    const filterDefs: { key: "home" | "firmware" | "model" | "function"; label: string; opts: { id: string; name: string }[]; val: string }[] = [
      ...
      { key: "function", label: "Function",     opts: this._functionOptions, val: this._filterFunction },
    ];
    ...
    return html`
      <div class="map-header">
        ...
        <div class="legend">
          ${legendItems.map(([color, label]) => html`
      ...
      <div class="filter-bar">
        ...
        <div class="layout-toggle">
          <button class=${`layout-btn${this._layoutMode === "spiral" ? " active" : ""}`}
                  @click=${() => this._onLayoutChange("spiral")}>⊛ Spiral</button>
          <button class=${`layout-btn${this._layoutMode === "linear" ? " active" : ""}`}
                  @click=${() => this._onLayoutChange("linear")}>⚌ Linéaire</button>
        </div>
      </div>

      <div class="canvas-wrap">
        <div class="viewcube-wrap"></div>
        <button class="overlay-btn fullscreen-btn" title="Toggle fullscreen"
                @click=${this._toggleFullscreen}>${this._isFullscreen ? "⊠" : "⛶"}</button>
        <button class="overlay-btn reset-btn"      title="Recadrer tout"
                @click=${this._resetCamera}>⟳</button>
        <button class=${`overlay-btn debug-btn${this._debugVisible ? " active" : ""}`}
                title="Console de debug" @click=${this._toggleDebug}>⚙</button>
        <div class=${`debug-overlay${this._debugVisible ? "" : " hidden"}`}></div>
        ${this._loading ? html`<div class="loading-overlay"><span>${i18n.t("map_loading")}</span></div>` : ""}
        <canvas></canvas>
        <div class="tooltip"></div>
      </div>

      <div class="stats-bar">
        <span><strong>${this._stats.homes}</strong>     ${i18n.t("homes")}</span>
        <span><strong>${this._stats.levels}</strong>    ${i18n.t("levels")}</span>
        <span><strong>${this._stats.rooms}</strong>     ${i18n.t("rooms")}</span>
        <span><strong>${this._stats.devices}</strong>   ${i18n.t("devices")}</span>
